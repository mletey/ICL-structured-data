...
modified by Mary Letey for 2025 sturctured In-Context Learning Project
"""
...
class fulltasksampler:
    def __init__(self, d, l, n, rho, Ctask) -> None:
        self.d = d # DIMENSION
    ...
    def __next__(self):
        ...
        ws = ws[..., np.newaxis] 
        ys = xs @ ws + self.rng.normal(loc=0, scale = np.sqrt(self.rho), size=(self.n, self.l+1, 1))
...
class finitetasksampler:
    def __init__(self, d, l, n, k, rho, Ctask) -> None:
        self.d = d # DIMENSION
        self.l = l # CONTEXT LENGTH
        self.n = n # NUMBER OF CONTEXTS
    ...
    def __next__(self):
        ...
        ys = xs @ ws + self.rng.normal(loc=0, scale = np.sqrt(self.rho), size=(self.n, context_length, 1))
