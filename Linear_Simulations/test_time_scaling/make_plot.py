...
filepath_m = f'runs/testime_d120_option0_5average/simulation_m.txt'
...
filepath_m = f'runs/testime_d120_option2_5average/simulation_m.txt'
with open(filepath_m, 'r') as f:
    contents = f.read().strip()
if contents.endswith(','):
    contents = contents[:-1]
...
plt.plot(alpha_TESTS, [ICL_error(Ctr0, Ctr0, tau, alpha, kappa, rho, 100, alpha_test) for alpha_test in alpha_TESTS], color=color_cycle[2], label = f'Isotropic task structure')
plt.scatter(alpha_TESTS, data_0[0][1],  color='#FAFAFA', edgecolors=color_cycle[2],s=150,linewidth=2.5,zorder=10)
plt.fill_between(alpha_TESTS, np.array(data_0[0][1]) - np.array(stds_0[0][1]), np.array(data_0[0][1]) + np.array(stds_0[0][1]), color=color_cycle[2], alpha = 0.2)
