...
plt.plot(kappas_theory, pretrain_theory, linestyle = '--', linewidth=4.8, color='#16762F', label = fr"$e_{{\mathrm{{bias}}}}$")

plt.plot(kappas_theory, np.array([ICL_error(Ctr, Ctr, tau, alpha, kappa, rho, 100) for kappa in kappas_theory]) - np.array(pretrain_theory), color='red', label = fr"$e_{{\mathrm{{align}}}}$, pretrain")
...
for i, plot_ind in enumerate(plot_inds):
    plt.plot(kappas_theory, np.array([ICL_error(Ctr, np.diag(spikevalue(d,0,signals[plot_ind])), tau, alpha, kappa, rho, 100) for kappa in kappas_theory]) - np.array(pretrain_theory), color=color_cycle[i+1], label =fr"$e_{{\mathrm{{align}}}}$, direction {signals[plot_ind]+1}/{d}")
